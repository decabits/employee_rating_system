...
var _default = (queryBuilderOptions = {}) => (from, fromAlias, resolveData, inOptions, withBuilder, context = {}, rootValue // eslint-disable-line flowtype/no-weak-types
  ...
  const rawCursorPrefix = reallyRawCursorPrefix && reallyRawCursorPrefix.filter(identity); // $FlowFixMe

  const queryBuilder = new _QueryBuilder.default( // $FlowFixMe
  queryBuilderOptions, context, rootValue);
...
OR\
  ...
  AND\
    ${sqlOldFilter}\
      ...
      } else if (cursorValue[0] === "natural" && (0, _isSafeInteger.default)(cursorValue[1]) && cursorValue[1] >= 0) {
        if (isAfter) {
    ...
    if (pgAggregateQuery && pgAggregateQuery.length) {
      const aggregateQueryBuilder = new _QueryBuilder.default( // $FlowFixMe
      queryBuilderOptions, context, rootValue);
