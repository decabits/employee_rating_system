  ...
  cb) {
    ...
    if (this.parentQueryBuilder) {
      ...
      this.parentQueryBuilder.beforeLock("select", () => {
        ...
        const allRequirements = this.data.liveConditions.reduce((memo, [_checkerGenerator, requirements]) => requirements ? Object.assign(memo, requirements) : memo, {}); // $FlowFixMe

        this.parentQueryBuilder.select(sql.fragment`\
...
${sql.join(Object.keys(allRequirements).map(key => sql.fragment`, ${sql.literal(key)}::text, ${allRequirements[key]}`), "")})`, "__live");
  ...
  selectIdentifiers(table) {
    ...
    this.select(sql.fragment`json_build_array(${sql.join(primaryKeys.map(key => sql.fragment`${this.getTableAlias()}.${sql.identifier(key.name)}`), ", ")})`, "__identifiers");
  ...
  lock(type) {
    ...
    if (type === "cursorComparator") {
      ...
      for (let i = 0; i < selects.length; i++) {
        const [valueOrGenerator, columnName] = selects[i]; // $FlowFixMe
